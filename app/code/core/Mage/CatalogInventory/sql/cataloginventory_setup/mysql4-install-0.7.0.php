...
CREATE TABLE IF NOT EXISTS `{$this->getTable('cataloginventory_stock')}` (
...
CREATE TABLE IF NOT EXISTS `{$this->getTable('cataloginventory_stock_item')}` (
    ...
    KEY `FK_CATALOGINVENTORY_STOCK_ITEM_STOCK` (`stock_id`),    CONSTRAINT `{$table_prefix}FK_CATALOGINVENTORY_STOCK_ITEM_PRODUCT` FOREIGN KEY (`product_id`) REFERENCES `{$this->getTable('catalog_product_entity')}` (`entity_id`) ON DELETE CASCADE ON UPDATE CASCADE,    CONSTRAINT `{$table_prefix}FK_CATALOGINVENTORY_STOCK_ITEM_STOCK` FOREIGN KEY (`stock_id`) REFERENCES `{$this->getTable('cataloginventory_stock')}` (`stock_id`) ON DELETE CASCADE ON UPDATE CASCADE
