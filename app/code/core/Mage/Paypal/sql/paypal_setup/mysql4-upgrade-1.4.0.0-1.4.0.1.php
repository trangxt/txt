...
DROP TABLE IF EXISTS `{$installer->getTable('paypal_settlement_report')}`;

CREATE TABLE IF NOT EXISTS `{$installer->getTable('paypal_settlement_report')}` (
...
DROP TABLE IF EXISTS `{$installer->getTable('paypal_settlement_report_row')}`;

CREATE TABLE IF NOT EXISTS `{$installer->getTable('paypal_settlement_report_row')}` (
...
ALTER TABLE `{$installer->getTable('paypal_settlement_report_row')}`
  ADD CONSTRAINT `{$table_prefix}FK_PAYPAL_SETTLEMENT_ROW_REPORT` FOREIGN KEY (`report_id`) REFERENCES `{$installer->getTable('paypal_settlement_report')}` (`report_id`) ON DELETE CASCADE ON UPDATE CASCADE;
