...
CREATE TABLE IF NOT EXISTS `{$installer->getTable('core_url_rewrite_temporary')}` (
...
DROP TABLE IF EXISTS `{$installer->getTable('core_url_rewrite_temporary')}`;
");
$conn->addConstraint($table_prefix.'FK_CORE_URL_REWRITE_CATEGORY', $installer->getTable('core_url_rewrite'), 'category_id',
    $installer->getTable('catalog_category_entity'), 'entity_id'
);
$conn->addConstraint($table_prefix.'FK_CORE_URL_REWRITE_PRODUCT', $installer->getTable('core_url_rewrite'), 'product_id',
