...
$installer->run("
DROP TABLE IF EXISTS `{$installer->getTable('catalog/eav_attribute')}`;
CREATE TABLE IF NOT EXISTS `{$installer->getTable('catalog/eav_attribute')}` (
  ...
  CONSTRAINT `{$table_prefix}FK_CATALOG_EAV_ATTRIBUTE_ID` FOREIGN KEY (`attribute_id`) REFERENCES `{$installer->getTable('eav/attribute')}` (`attribute_id`) ON DELETE CASCADE ON UPDATE CASCADE
