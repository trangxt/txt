...
$installer->run("
    CREATE TABLE IF NOT EXISTS `{$installer->getTable('catalogindex_eav_tmp')}` (
    ...
    CREATE TABLE IF NOT EXISTS `{$installer->getTable('catalogindex_eav')}` (
        ...
        KEY `FK_CATALOGINDEX_EAV_STORE` (`store_id`),    CONSTRAINT `{$table_prefix}FK_CATALOGINDEX_EAV_ATTRIBUTE` FOREIGN KEY (`attribute_id`)
            REFERENCES `{$installer->getTable('eav_attribute')}` (`attribute_id`) ON DELETE CASCADE ON UPDATE CASCADE,    CONSTRAINT `{$table_prefix}FK_CATALOGINDEX_EAV_ENTITY` FOREIGN KEY (`entity_id`)
            REFERENCES `{$installer->getTable('catalog_product_entity')}` (`entity_id`) ON DELETE CASCADE ON UPDATE CASCADE,    CONSTRAINT `{$table_prefix}FK_CATALOGINDEX_EAV_STORE` FOREIGN KEY (`store_id`)
    ...
    CREATE TABLE IF NOT EXISTS `{$installer->getTable('catalogindex_price_tmp')}` (
      `store_id` smallint(5) unsigned NOT NULL default '0',
      `entity_id` int(10) unsigned NOT NULL default '0',
      `attribute_id` smallint(5) unsigned NOT NULL default '0',
    ...
    CREATE TABLE IF NOT EXISTS `{$installer->getTable('catalogindex_price')}` (
      ...
      KEY `IDX_RANGE_VALUE` (`store_id`, `entity_id`,`attribute_id`,`customer_group_id`,`value`),    CONSTRAINT `{$table_prefix}FK_CATALOGINDEX_PRICE_ATTRIBUTE` FOREIGN KEY (`attribute_id`)
        REFERENCES `{$installer->getTable('eav_attribute')}` (`attribute_id`) ON DELETE CASCADE ON UPDATE CASCADE,    CONSTRAINT `{$table_prefix}FK_CATALOGINDEX_PRICE_ENTITY` FOREIGN KEY (`entity_id`)
        REFERENCES `{$installer->getTable('catalog_product_entity')}` (`entity_id`) ON DELETE CASCADE ON UPDATE CASCADE,    CONSTRAINT `{$table_prefix}FK_CATALOGINDEX_PRICE_STORE` FOREIGN KEY (`store_id`)
        REFERENCES `{$installer->getTable('core_store')}` (`store_id`) ON DELETE CASCADE ON UPDATE CASCADE
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8;
