...
CREATE TABLE IF NOT EXISTS {$this->getTable('api_assert')} (
...
CREATE TABLE IF NOT EXISTS {$this->getTable('api_role')} (
...
CREATE TABLE IF NOT EXISTS {$this->getTable('api_rule')} (
  ...
  KEY `role_id` (`role_id`,`resource_id`),    CONSTRAINT `{$table_prefix}FK_api_rule` FOREIGN KEY (`role_id`) REFERENCES {$this->getTable('api_role')} (`role_id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='Api ACL Rules';

-- DROP TABLE IF EXISTS {$this->getTable('api_user')};
CREATE TABLE IF NOT EXISTS {$this->getTable('api_user')} (
