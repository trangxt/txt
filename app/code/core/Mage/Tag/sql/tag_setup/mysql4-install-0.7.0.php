...
CREATE TABLE IF NOT EXISTS {$this->getTable('tag')} (
...
CREATE TABLE IF NOT EXISTS {$this->getTable('tag_relation')} (
  ...
  KEY `FK_TAG_RELATION_STORE` (`store_id`),    CONSTRAINT `{$table_prefix}FK_TAG_RELATION_PRODUCT` FOREIGN KEY (`product_id`) REFERENCES {$this->getTable('catalog_product_entity')} (`entity_id`) ON DELETE CASCADE ON UPDATE CASCADE,
...
CREATE TABLE IF NOT EXISTS {$this->getTable('tag_summary')} (
   `tag_id` int(11) unsigned NOT NULL default '0',
