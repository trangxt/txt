...
CREATE TABLE IF NOT EXISTS {$this->getTable('review')} (
  ...
  KEY `FK_REVIEW_PARENT_PRODUCT` (`entity_pk_value`),    CONSTRAINT `{$table_prefix}FK_REVIEW_ENTITY` FOREIGN KEY (`entity_id`) REFERENCES {$this->getTable('review_entity')} (`entity_id`) ON DELETE CASCADE ON UPDATE CASCADE,    CONSTRAINT `{$table_prefix}FK_REVIEW_PARENT_PRODUCT` FOREIGN KEY (`entity_pk_value`) REFERENCES {$this->getTable('catalog_product_entity')} (`entity_id`) ON DELETE CASCADE ON UPDATE CASCADE,    CONSTRAINT `{$table_prefix}FK_REVIEW_STATUS` FOREIGN KEY (`status_id`) REFERENCES {$this->getTable('review_status')} (`status_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='Review base information';

-- DROP TABLE IF EXISTS {$this->getTable('review_detail')};
CREATE TABLE IF NOT EXISTS {$this->getTable('review_detail')} (
  ...
  KEY `FK_REVIEW_DETAIL_REVIEW` (`review_id`),    CONSTRAINT `{$table_prefix}FK_REVIEW_DETAIL_REVIEW` FOREIGN KEY (`review_id`) REFERENCES {$this->getTable('review')} (`review_id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='Review detail information';

-- DROP TABLE IF EXISTS {$this->getTable('review_entity')};
CREATE TABLE IF NOT EXISTS {$this->getTable('review_entity')} (
...
CREATE TABLE IF NOT EXISTS {$this->getTable('review_entity_summary')} (
...
CREATE TABLE IF NOT EXISTS {$this->getTable('review_status')} (
...
CREATE TABLE IF NOT EXISTS `{$this->getTable('review_store')}` (
  `review_id` bigint(20) unsigned NOT NULL,
  `store_id` smallint(5) unsigned NOT NULL,
  PRIMARY KEY  (`review_id`,`store_id`),    CONSTRAINT `{$table_prefix}FK_REVIEW_STORE_REVIEW` FOREIGN KEY (`review_id`) REFERENCES `{$this->getTable('review')}` (`review_id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
