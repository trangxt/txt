...
$installer->run("
DROP TABLE IF EXISTS `{$installer->getTable('sales/order_aggregated_created')}`;
CREATE TABLE IF NOT EXISTS `{$installer->getTable('sales/order_aggregated_created')}` (
  ...
  CONSTRAINT `{$table_prefix}_FK_SALES_ORDER_AGGREGATED_CREATED` FOREIGN KEY (`store_id`) REFERENCES `{$installer->getTable('core_store')}` (`store_id`) ON DELETE SET NULL ON UPDATE CASCADE
