...
CREATE TABLE IF NOT EXISTS {$this->getTable('sales_order')} (
  ...
  KEY `FK_sales_order_store` (`store_id`),    CONSTRAINT `{$table_prefix}FK_SALE_ORDER_STORE` FOREIGN KEY (`store_id`) REFERENCES `{$this->getTable('core_store')}` (`store_id`) ON DELETE SET NULL ON UPDATE CASCADE,    CONSTRAINT `{$table_prefix}FK_SALE_ORDER_TYPE` FOREIGN KEY (`entity_type_id`) REFERENCES `{$this->getTable('eav_entity_type')}` (`entity_type_id`) ON DELETE CASCADE ON UPDATE CASCADE
