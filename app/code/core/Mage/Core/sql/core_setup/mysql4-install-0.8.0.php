...
$installer->run("
CREATE TABLE IF NOT EXISTS `{$installer->getTable('core_resource')}` (
  `code` varchar(50) NOT NULL default '',
  `version` varchar(50) NOT NULL default '',
  PRIMARY KEY  (`code`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='Resource version registry';

CREATE TABLE IF NOT EXISTS `{$installer->getTable('core_website')}` (
...
CREATE TABLE IF NOT EXISTS `{$installer->getTable('core_store_group')}` (
...
CREATE TABLE IF NOT EXISTS `{$installer->getTable('core_store')}` (
...
CREATE TABLE IF NOT EXISTS `{$installer->getTable('core_config_data')}` (
...
CREATE TABLE IF NOT EXISTS `{$installer->getTable('core_email_template')}` (
...
CREATE TABLE IF NOT EXISTS `{$installer->getTable('core_layout_update')}` (
...
CREATE TABLE IF NOT EXISTS `{$installer->getTable('core_layout_link')}` (
...
CREATE TABLE IF NOT EXISTS `{$installer->getTable('core_session')}` (
...
CREATE TABLE IF NOT EXISTS `{$installer->getTable('core_translate')}` (
...
CREATE TABLE IF NOT EXISTS `{$installer->getTable('core_url_rewrite')}` (
...
CREATE TABLE IF NOT EXISTS `{$installer->getTable('core_url_rewrite_tag')}` (
...
CREATE TABLE IF NOT EXISTS `{$installer->getTable('design_change')}` (
...
ALTER TABLE `{$installer->getTable('core_store_group')}`
  ADD    CONSTRAINT `{$table_prefix}FK_STORE_GROUP_WEBSITE` FOREIGN KEY (`website_id`) REFERENCES `{$installer->getTable('core_website')}` (`website_id`) ON DELETE CASCADE ON UPDATE CASCADE;

ALTER TABLE `{$installer->getTable('core_store')}`
  ADD    CONSTRAINT `{$table_prefix}FK_STORE_GROUP_STORE` FOREIGN KEY (`group_id`) REFERENCES `{$installer->getTable('core_store_group')}` (`group_id`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD    CONSTRAINT `{$table_prefix}FK_STORE_WEBSITE` FOREIGN KEY (`website_id`) REFERENCES `{$installer->getTable('core_website')}` (`website_id`) ON DELETE CASCADE ON UPDATE CASCADE;

ALTER TABLE `{$installer->getTable('core_layout_link')}`
  ADD    CONSTRAINT `{$table_prefix}FK_CORE_LAYOUT_LINK_STORE` FOREIGN KEY (`store_id`) REFERENCES `{$installer->getTable('core_store')}` (`store_id`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD    CONSTRAINT `{$table_prefix}FK_CORE_LAYOUT_LINK_UPDATE` FOREIGN KEY (`layout_update_id`) REFERENCES `{$installer->getTable('core_layout_update')}` (`layout_update_id`) ON DELETE CASCADE ON UPDATE CASCADE;

ALTER TABLE `{$installer->getTable('core_session')}`
  ADD    CONSTRAINT `{$table_prefix}FK_SESSION_WEBSITE` FOREIGN KEY (`website_id`) REFERENCES `{$installer->getTable('core_website')}` (`website_id`) ON DELETE CASCADE ON UPDATE CASCADE;

ALTER TABLE `{$installer->getTable('core_translate')}`
  ADD    CONSTRAINT `{$table_prefix}FK_CORE_TRANSLATE_STORE` FOREIGN KEY (`store_id`) REFERENCES `{$installer->getTable('core_store')}` (`store_id`) ON DELETE CASCADE ON UPDATE CASCADE;

ALTER TABLE `{$installer->getTable('core_url_rewrite')}`
  ADD    CONSTRAINT `{$table_prefix}FK_CORE_URL_REWRITE_STORE` FOREIGN KEY (`store_id`) REFERENCES `{$installer->getTable('core_store')}` (`store_id`) ON DELETE CASCADE ON UPDATE CASCADE;

ALTER TABLE `{$installer->getTable('core_url_rewrite_tag')}`
  ADD    CONSTRAINT `{$table_prefix}FK_CORE_URL_REWRITE_TAG_URL_REWRITE` FOREIGN KEY (`url_rewrite_id`) REFERENCES `{$installer->getTable('core_url_rewrite')}` (`url_rewrite_id`) ON DELETE CASCADE ON UPDATE CASCADE;

ALTER TABLE `{$installer->getTable('design_change')}`
  ADD    CONSTRAINT `{$table_prefix}FK_DESIGN_CHANGE_STORE` FOREIGN KEY (`store_id`) REFERENCES `{$installer->getTable('core_store')}` (`store_id`) ON DELETE CASCADE ON UPDATE CASCADE;
