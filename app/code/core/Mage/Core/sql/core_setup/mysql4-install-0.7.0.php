...
CREATE TABLE IF NOT EXISTS {$this->getTable('core_config_data')} (
...
CREATE TABLE IF NOT EXISTS {$this->getTable('core_email_template')} (
...
CREATE TABLE IF NOT EXISTS {$this->getTable('core_language')} (
...
CREATE TABLE IF NOT EXISTS {$this->getTable('core_resource')} (
...
CREATE TABLE IF NOT EXISTS {$this->getTable('core_session')} (
  `session_id` varchar(255) NOT NULL default '',
  `website_id` smallint(5) unsigned default NULL,
  `session_expires` int(10) unsigned NOT NULL default '0',
  `session_data` text NOT NULL,
  PRIMARY KEY  (`session_id`),
  KEY `FK_SESSION_WEBSITE` (`website_id`),    CONSTRAINT `{$table_prefix}FK_SESSION_WEBSITE` FOREIGN KEY (`website_id`) REFERENCES {$this->getTable('core_website')} (`website_id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='Session data store';

-- DROP TABLE IF EXISTS {$this->getTable('core_store')};
CREATE TABLE IF NOT EXISTS {$this->getTable('core_store')} (
  ...
  KEY `is_active` (`is_active`,`sort_order`),    CONSTRAINT `{$table_prefix}FK_STORE_LANGUAGE` FOREIGN KEY (`language_code`) REFERENCES {$this->getTable('core_language')} (`language_code`) ON DELETE SET NULL ON UPDATE SET NULL,    CONSTRAINT `{$table_prefix}FK_STORE_WEBSITE` FOREIGN KEY (`website_id`) REFERENCES {$this->getTable('core_website')} (`website_id`) ON DELETE SET NULL ON UPDATE SET NULL
...
CREATE TABLE IF NOT EXISTS {$this->getTable('core_translate')} (
  ...
  KEY `FK_CORE_TRANSLATE_STORE` (`store_id`),    CONSTRAINT `{$table_prefix}FK_CORE_TRANSLATE_STORE` FOREIGN KEY (`store_id`) REFERENCES {$this->getTable('core_store')} (`store_id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='Translation data';


-- DROP TABLE IF EXISTS {$this->getTable('core_website')};
CREATE TABLE IF NOT EXISTS {$this->getTable('core_website')} (
...
CREATE TABLE IF NOT EXISTS {$this->getTable('core_layout_update')} (
...
CREATE TABLE IF NOT EXISTS {$this->getTable('core_layout_link')} (
  ...
  KEY `FK_core_layout_link_update` (`layout_update_id`),    CONSTRAINT `{$table_prefix}FK_core_layout_link_store` FOREIGN KEY (`store_id`) REFERENCES {$this->getTable('core_store')} (`store_id`) ON DELETE CASCADE ON UPDATE CASCADE,    CONSTRAINT `{$table_prefix}FK_core_layout_link_update` FOREIGN KEY (`layout_update_id`) REFERENCES {$this->getTable('core_layout_update')} (`layout_update_id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

-- DROP TABLE if exists {$this->getTable('core_url_rewrite')};
CREATE TABLE IF NOT EXISTS {$this->getTable('core_url_rewrite')} (
...
CREATE TABLE IF NOT EXISTS {$this->getTable('core_url_rewrite_tag')} (
...
CREATE TABLE IF NOT EXISTS {$this->getTable('design_change')} (
...
ALTER TABLE {$this->getTable('design_change')}
  ADD    CONSTRAINT `{$table_prefix}FK_DESIGN_CHANGE_STORE`
   FOREIGN KEY (`store_id`)
