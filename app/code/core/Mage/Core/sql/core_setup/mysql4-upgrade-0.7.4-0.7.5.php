...
CREATE TABLE IF NOT EXISTS {$this->getTable('core_store_group')} (
  ...
  KEY (`default_store_id`),    CONSTRAINT `{$table_prefix}FK_STORE_GROUP_WEBSITE` FOREIGN KEY (`website_id`) REFERENCES {$this->getTable('core_website')} (`website_id`) ON DELETE CASCADE ON UPDATE CASCADE
...
ALTER TABLE {$this->getTable('core_store')}
    ADD    CONSTRAINT `{$table_prefix}FK_STORE_GROUP_STORE` FOREIGN KEY (`group_id`)
