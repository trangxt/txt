...
CREATE TABLE IF NOT EXISTS {$this->getTable('tax_class')} (
...
CREATE TABLE IF NOT EXISTS {$this->getTable('tax_rate')} (
...
CREATE TABLE IF NOT EXISTS {$this->getTable('tax_rate_data')} (
  ...
  UNIQUE idx_rate_rate_type (tax_rate_id, rate_type_id),    CONSTRAINT `{$table_prefix}FK_TAX_RATE_DATA_TAX_RATE` FOREIGN KEY (`tax_rate_id`) REFERENCES {$this->getTable('tax_rate')} (`tax_rate_id`) ON DELETE CASCADE ON UPDATE CASCADE,    CONSTRAINT `{$table_prefix}FK_TAX_RATE_DATE_TAX_RATE_TYPE` FOREIGN KEY (`rate_type_id`) REFERENCES {$this->getTable('tax_rate_type')} (`type_id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
CREATE TABLE IF NOT EXISTS {$this->getTable('tax_rate_type')} (
...
CREATE TABLE IF NOT EXISTS {$this->getTable('tax_rule')} (
  ...
  KEY `tax_rate_id` (`tax_rate_type_id`),    CONSTRAINT `{$table_prefix}FK_TAX_RULE_TAX_CLASS_CUSTOMER` FOREIGN KEY (`tax_customer_class_id`) REFERENCES {$this->getTable('tax_class')} (`class_id`) ON DELETE CASCADE ON UPDATE CASCADE,    CONSTRAINT `{$table_prefix}FK_TAX_RULE_TAX_CLASS_PRODUCT` FOREIGN KEY (`tax_product_class_id`) REFERENCES {$this->getTable('tax_class')} (`class_id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
