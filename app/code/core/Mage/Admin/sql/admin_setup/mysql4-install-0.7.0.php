...
CREATE TABLE IF NOT EXISTS {$this->getTable('admin_assert')} (
...
CREATE TABLE IF NOT EXISTS {$this->getTable('admin_role')} (
...
CREATE TABLE IF NOT EXISTS {$this->getTable('admin_rule')} (
  ...
  KEY `role_id` (`role_id`,`resource_id`),    CONSTRAINT `{$table_prefix}FK_admin_rule` FOREIGN KEY (`role_id`) REFERENCES {$this->getTable('admin_role')} (`role_id`) ON DELETE CASCADE ON UPDATE CASCADE
...
CREATE TABLE IF NOT EXISTS {$this->getTable('admin_user')} (
