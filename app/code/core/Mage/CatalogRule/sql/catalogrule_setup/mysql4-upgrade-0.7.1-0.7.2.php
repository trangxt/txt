...
if ($conn->tableColumnExists($ruleProductTable, 'store_id')) {
    ...
    $conn->raw_query("ALTER TABLE `$ruleProductTable` ADD    CONSTRAINT `{$table_prefix}FK_catalogrule_product_website` FOREIGN KEY (`website_id`) REFERENCES `{$this->getTable('core_website')}` (`website_id`) ON DELETE CASCADE ON UPDATE CASCADE");
}
...
if ($conn->tableColumnExists($ruleProductPriceTable, 'store_id')) {
    ...
    $conn->raw_query("ALTER TABLE `$ruleProductPriceTable` ADD    CONSTRAINT `{$table_prefix}FK_catalogrule_product_price_website` FOREIGN KEY (`website_id`) REFERENCES `{$this->getTable('core_website')}` (`website_id`) ON DELETE CASCADE ON UPDATE CASCADE");
}
