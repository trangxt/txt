...
CREATE TABLE IF NOT EXISTS `{$installer->getTable('newsletter_problem')}` (
  ...
  KEY `FK_PROBLEM_QUEUE` (`queue_id`),    CONSTRAINT `{$table_prefix}FK_PROBLEM_QUEUE` FOREIGN KEY (`queue_id`) REFERENCES `{$installer->getTable('newsletter_queue')}` (`queue_id`),    CONSTRAINT `{$table_prefix}FK_PROBLEM_SUBSCRIBER` FOREIGN KEY (`subscriber_id`) REFERENCES `{$installer->getTable('newsletter_subscriber')}` (`subscriber_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='Newsletter problems';

-- DROP TABLE IF EXISTS `{$installer->getTable('newsletter_queue')}`;
CREATE TABLE IF NOT EXISTS `{$installer->getTable('newsletter_queue')}` (
  ...
  KEY `FK_QUEUE_TEMPLATE` (`template_id`),    CONSTRAINT `{$table_prefix}FK_QUEUE_TEMPLATE` FOREIGN KEY (`template_id`) REFERENCES `{$installer->getTable('newsletter_template')}` (`template_id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='Newsletter queue';

-- DROP TABLE IF EXISTS `{$installer->getTable('newsletter_queue_link')}`;
CREATE TABLE IF NOT EXISTS `{$installer->getTable('newsletter_queue_link')}` (
  ...
  KEY `FK_QUEUE_LINK_QUEUE` (`queue_id`),    CONSTRAINT `{$table_prefix}FK_QUEUE_LINK_QUEUE` FOREIGN KEY (`queue_id`) REFERENCES `{$installer->getTable('newsletter_queue')}` (`queue_id`) ON DELETE CASCADE,    CONSTRAINT `{$table_prefix}FK_QUEUE_LINK_SUBSCRIBER` FOREIGN KEY (`subscriber_id`) REFERENCES `{$installer->getTable('newsletter_subscriber')}` (`subscriber_id`) ON DELETE CASCADE
...
CREATE TABLE IF NOT EXISTS `{$installer->getTable('newsletter_queue_store_link')}` (
  `queue_id` int(7) unsigned NOT NULL default '0',
  `store_id` smallint(5) unsigned NOT NULL default '0',
  PRIMARY KEY  (`queue_id`,`store_id`),
  KEY `FK_NEWSLETTER_QUEUE_STORE_LINK_STORE` (`store_id`),    CONSTRAINT `{$table_prefix}FK_NEWSLETTER_QUEUE_STORE_LINK_STORE` FOREIGN KEY (`store_id`) REFERENCES `{$installer->getTable('core_store')}` (`store_id`) ON DELETE CASCADE ON UPDATE CASCADE,    CONSTRAINT `{$table_prefix}FK_LINK_QUEUE` FOREIGN KEY (`queue_id`) REFERENCES `{$installer->getTable('newsletter_queue')}` (`queue_id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

-- DROP TABLE IF EXISTS `{$installer->getTable('newsletter_subscriber')}`;
CREATE TABLE IF NOT EXISTS `{$installer->getTable('newsletter_subscriber')}` (
  ...
  KEY `FK_NEWSLETTER_SUBSCRIBER_STORE` (`store_id`),    CONSTRAINT `{$table_prefix}FK_NEWSLETTER_SUBSCRIBER_STORE` FOREIGN KEY (`store_id`) REFERENCES `{$installer->getTable('core_store')}` (`store_id`) ON DELETE SET NULL ON UPDATE CASCADE
...
CREATE TABLE IF NOT EXISTS `{$installer->getTable('newsletter_template')}` (
