...
CREATE TABLE IF NOT EXISTS {$this->getTable('salesrule')} (
...
CREATE TABLE IF NOT EXISTS {$this->getTable('salesrule_customer')} (
  ...
  KEY `customer_id` (`customer_id`,`rule_id`),    CONSTRAINT `{$table_prefix}FK_salesrule_customer_id` FOREIGN KEY (`customer_id`) REFERENCES {$this->getTable('customer_entity')} (`entity_id`) ON DELETE CASCADE ON UPDATE CASCADE,    CONSTRAINT `{$table_prefix}FK_salesrule_customer_rule` FOREIGN KEY (`rule_id`) REFERENCES {$this->getTable('salesrule')} (`rule_id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

-- DROP TABLE IF EXISTS {$this->getTable('salesrule_product')};
CREATE TABLE IF NOT EXISTS {$this->getTable('salesrule_product')} (
  ...
  KEY `FK_salesrule_product_customergroup` (`customer_group_id`),    CONSTRAINT `{$table_prefix}FK_salesrule_product_customergroup` FOREIGN KEY (`customer_group_id`) REFERENCES {$this->getTable('customer_group')} (`customer_group_id`) ON DELETE CASCADE ON UPDATE CASCADE,    CONSTRAINT `{$table_prefix}FK_salesrule_product_rule` FOREIGN KEY (`rule_id`) REFERENCES {$this->getTable('salesrule')} (`rule_id`) ON DELETE CASCADE ON UPDATE CASCADE,    CONSTRAINT `{$table_prefix}FK_salesrule_product_store` FOREIGN KEY (`store_id`) REFERENCES {$this->getTable('core_store')} (`store_id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

-- DROP TABLE IF EXISTS {$this->getTable('salesrule_product_action')};
CREATE TABLE IF NOT EXISTS {$this->getTable('salesrule_product_action')} (
