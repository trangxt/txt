...
CREATE TABLE IF NOT EXISTS `{$installer->getTable('catalogsearch_fulltext')}` (
...
DROP TABLE IF EXISTS `{$installer->getTable('catalogsearch_result')}`;

CREATE TABLE IF NOT EXISTS `{$installer->getTable('catalogsearch_result')}` (
  ...
  KEY `IDX_RELEVANCE` (`query_id`, `relevance`),    CONSTRAINT `{$table_prefix}FK_CATALOGSEARCH_RESULT_QUERY` FOREIGN KEY (`query_id`) REFERENCES `{$installer->getTable('catalogsearch_query')}` (`query_id`) ON DELETE CASCADE ON UPDATE CASCADE,    CONSTRAINT `{$table_prefix}FK_CATALOGSEARCH_RESULT_CATALOG_PRODUCT` FOREIGN KEY (`product_id`) REFERENCES `{$installer->getTable('catalog_product_entity')}` (`entity_id`) ON DELETE CASCADE ON UPDATE CASCADE
...
$connection->addConstraint($table_prefix.'FK_CATALOGSEARCH_QUERY_STORE',
...
//$connection->dropKey($installer->getTable('catalogsearch_query'), 'FK_search_query');
