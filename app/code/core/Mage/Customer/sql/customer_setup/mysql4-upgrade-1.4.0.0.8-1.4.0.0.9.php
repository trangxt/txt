...
$installer->run("
DROP TABLE IF EXISTS `{$installer->getTable('customer/eav_attribute_website')}`;
CREATE TABLE `{$installer->getTable('customer/eav_attribute_website')}` (
  ...
  CONSTRAINT `{$table_prefix}FK_CUSTOMER_EAV_ATTRIBUTE_WEBSITE_ATTRIBUTE_EAV_ATT` FOREIGN KEY (`attribute_id`)
    REFERENCES `{$installer->getTable('eav/attribute')}` (`attribute_id`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `{$table_prefix}FK_CUSTOMER_EAV_ATTRIBUTE_WEBSITE_WEBSITE_CORE_WEBSITE` FOREIGN KEY (`website_id`)
