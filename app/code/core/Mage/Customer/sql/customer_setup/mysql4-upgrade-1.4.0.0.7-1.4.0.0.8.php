...
$installer->run("
DROP TABLE IF EXISTS `{$installer->getTable('customer/form_attribute')}`;
CREATE TABLE `{$installer->getTable('customer/form_attribute')}` (
  ...
  CONSTRAINT `{$table_prefix}FK_CUSTOMER_FORM_ATTRIBUTE_ATTRIBUTE` FOREIGN KEY (`attribute_id`) REFERENCES `{$installer->getTable('eav_attribute')}` (`attribute_id`) ON DELETE CASCADE ON UPDATE CASCADE
