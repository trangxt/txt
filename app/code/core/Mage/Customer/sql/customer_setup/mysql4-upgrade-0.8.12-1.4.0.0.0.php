...
CREATE TABLE IF NOT EXISTS `{$installer->getTable('customer/eav_attribute')}` (
  ...
  CONSTRAINT `{$table_prefix}FK_CUSTOMER_EAV_ATTRIBUTE_ID` FOREIGN KEY (`attribute_id`) REFERENCES `{$installer->getTable('eav/attribute')}` (`attribute_id`) ON DELETE CASCADE ON UPDATE CASCADE
