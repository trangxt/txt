...
CREATE TABLE IF NOT EXISTS `{$installer->getTable('directory_country')}` (
...
CREATE TABLE IF NOT EXISTS `{$installer->getTable('directory_country_format')}` (
...
CREATE TABLE IF NOT EXISTS `{$installer->getTable('directory_country_region')}` (
...
CREATE TABLE IF NOT EXISTS `{$installer->getTable('directory_country_region_name')}` (
  `locale` varchar(8) NOT NULL default '',
  `region_id` mediumint(8) unsigned NOT NULL default '0',
  `name` varchar(64) NOT NULL default '',
  PRIMARY KEY  (`locale`,`region_id`),
  KEY `FK_DIRECTORY_REGION_NAME_REGION` (`region_id`),    CONSTRAINT `{$table_prefix}FK_DIRECTORY_REGION_NAME_REGION` FOREIGN KEY (`region_id`) REFERENCES {$installer->getTable('directory_country_region')} (`region_id`) ON DELETE CASCADE ON UPDATE CASCADE
...
CREATE TABLE IF NOT EXISTS `{$installer->getTable('directory_currency_rate')}` (
