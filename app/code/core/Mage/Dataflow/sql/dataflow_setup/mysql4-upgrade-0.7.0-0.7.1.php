...
CREATE TABLE IF NOT EXISTS {$this->getTable('dataflow_profile')} (
...
CREATE TABLE IF NOT EXISTS {$this->getTable('dataflow_profile_history')} (
  ...
  KEY `FK_dataflow_profile_history` (`profile_id`),    CONSTRAINT `{$table_prefix}FK_dataflow_profile_history` FOREIGN KEY (`profile_id`) REFERENCES {$this->getTable('dataflow_profile')} (`profile_id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
");
